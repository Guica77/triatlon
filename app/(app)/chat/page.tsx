...
export default async function AthleteChatPage() {
  ...
  return (
    ...
    >
      ...
      {/* Keep the product identity centred within the same mobile column as the content. */}
      <header className="shrink-0 border-b border-border-subtle bg-white/95 backdrop-blur-md pt-[env(safe-area-inset-top)]">
        <div className="mx-auto flex w-full max-w-4xl items-center justify-between gap-3 px-4 py-3 sm:px-6 sm:py-4">
          <div className="flex min-w-0 items-center gap-3">
            <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl border border-swim/40 bg-swim-subtle shadow-sm">
            ...
            <div className="min-w-0">
              ...
              <p className="truncate text-[11px] font-semibold text-text-muted sm:text-xs">Comunicación directa y resolución de dudas</p>
            </div>
          </div>

          <Link href="/dashboard" className="shrink-0" aria-label="Volver al Dashboard">
            <AnimatedButton variant="ghost" size="icon" className="h-10 w-10 rounded-full border border-zinc-200 text-zinc-600 hover:bg-zinc-100 hover:text-zinc-900">
      ...
      {/* Main chat viewport */}
      <main className="mx-auto flex min-h-0 w-full max-w-4xl flex-1 flex-col overflow-hidden px-0 pb-0 pt-2 sm:px-6 sm:pt-8">
        <ChatView
