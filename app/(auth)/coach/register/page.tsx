...
import { registerCoach, getOAuthUrl } from '../../actions';
...
export default function CoachRegisterPage() {
  ...
  async function handleGoogleLogin() {
    setError(null);
    // Securely sets the cookie as 'coach' on the server via the Server Action, then redirects
    const res = await getOAuthUrl('google', 'coach');
    if (res?.url) {
          ...
          {!successMessage && (
            <>
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1.5">
                  ...
                  <input 
                    ...
                    required 
                    placeholder="Nombre"
                    className="w-full bg-zinc-900/50 border border-zinc-800 rounded-xl p-3.5 text-sm text-white placeholder-zinc-600 outline-none focus:border-orange-500/50 focus:bg-zinc-900 transition-all"
                  />
                </div>
                <div className="space-y-1.5">
              ...
              <button 
                ...
                disabled={loading}
              >
                {loading ? 'Procesando...' : 'Crear mi Panel de Coach'}
              </button>
            </>
          )}
        </form>
