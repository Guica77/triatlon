...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!input || typeof input !== 'object' || !('identityToken' in input) ||
        typeof input.identityToken !== 'string' || !input.identityToken || input.identityToken.length > 12288) {
      return reply({ error: 'Credencial de Google inválida' }, 400)
    }
