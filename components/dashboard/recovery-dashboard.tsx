...
export function RecoveryDashboard({ analysis }: RecoveryDashboardProps) {
  ...
  const trendColor = weeklyTrend === 'improving' ? 'text-emerald-400' : weeklyTrend === 'declining' ? 'text-red-400' : 'text-zinc-500'
    ...
    <div className="bg-zinc-900/80 border border-zinc-800 rounded-2xl p-5">
      ...
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-sm font-bold text-white">Estado de Recuperación</h3>
            <p className="text-[10px] text-zinc-500 font-medium">Análisis multifactores</p>
          </div>
        </div>
      ...
      <div className="flex items-center gap-6 mb-5">
        ...
        <div className="flex-1">
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-sm font-bold text-white">{suggestedToday.label}</span>
          </div>
          <p className="text-[11px] text-zinc-400 leading-relaxed">{suggestedToday.description}</p>
          {readinessForHighIntensity && (
            <div className="mt-2 flex items-center gap-1.5">
              <Zap className="w-3 h-3 text-emerald-400" />
              <span className="text-[10px] text-emerald-400 font-bold">Listo para alta intensidad</span>
            </div>
          )}
        </div>
      </div>
      ...
      {overtrainingRisk > 30 && (
        <div className={cn(
          ...
          <div className="flex items-center gap-2">
            <AlertTriangle className={cn('w-4 h-4', overtrainingRisk >= 60 ? 'text-red-400' : 'text-amber-400')} />
            <span className={cn('text-xs font-bold', overtrainingRisk >= 60 ? 'text-red-400' : 'text-amber-400')}>
      ...
      {recommendations.length > 0 && (
        <div>
          <p className="text-[10px] font-bold text-zinc-500 uppercase tracking-wider mb-2">Recomendaciones</p>
    ...
    <div className="flex items-center gap-3">
      <div className={cn(
        ...
        factor.status === 'good' ? 'bg-emerald-500/10 text-emerald-400' :
          factor.status === 'warning' ? 'bg-amber-500/10 text-amber-400' :
      ...
      <div className="flex-1 min-w-0">
        <div className="flex justify-between text-[10px] mb-0.5">
          <span className="text-zinc-400 font-medium">{factor.name}</span>
          <span className={cn('font-bold', factor.status === 'good' ? 'text-emerald-400' : factor.status === 'warning' ? 'text-amber-400' : 'text-red-400')}>
            {factor.score}%
          </span>
        </div>
        ...
        <p className="text-[9px] text-zinc-500 mt-0.5">{factor.detail}</p>
      </div>
    </div>
  )
}