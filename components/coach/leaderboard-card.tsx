...
export function LeaderboardCard({ entries }: LeaderboardCardProps) {
  ...
  const getMedalColor = (index: number) => {
    ...
    if (index === 1) return 'text-text-muted bg-bg-hover/10 border-border-default/20'
    if (index === 2) return 'text-amber-600 bg-amber-700/10 border-amber-700/20'
    return 'text-text-muted bg-bg-card/50 border-border-default/50'
  }

  return (
    <div className="bg-bg-card/80 border border-border-default rounded-2xl p-5">
      <div className="flex items-center gap-3 mb-4">
        ...
        <div>
          ...
          <p className="text-[10px] text-text-muted font-medium">Ranking por volumen de entrenamiento</p>
        </div>
      </div>
      ...
      {sorted.length === 0 ? (
        <div className="text-center py-6">
          ...
          <p className="text-xs text-text-muted font-medium">Aún no hay datos esta semana</p>
        </div>
      ) : (
        <div className="space-y-2">
          {sorted.slice(0, 10).map((entry, i) => (
            ...
            >
              ...
              {/* Avatar */}
              <div className="w-8 h-8 rounded-full bg-bg-card flex items-center justify-center text-xs font-bold text-white shrink-0">
              ...
              <div className="text-right shrink-0">
                ...
                <p className="text-[9px] text-text-muted font-bold uppercase">TSS</p>
              </div>
            </motion.div>
          ))}
        </div>
