...
export function NotificationTestCard() {
  ...
  return (
    <div className="p-5 rounded-2xl bg-surface-card border border-border-default shadow-card relative h-full flex flex-col justify-between">
      <div>
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            ...
            <p className="text-[10px] sm:text-xs text-text-secondary font-medium">Prueba la recepción de alertas</p>
          </div>
        </div>
        
        {error && (
          <div className="mb-3 p-3 rounded-xl bg-danger/10 border border-danger/20 text-xs text-danger font-semibold leading-normal">
            {error}
          </div>
        )}

        {success && (
          <div className="mb-3 p-3 rounded-xl bg-bike/10 border border-bike/30 text-xs text-bike font-semibold flex items-center gap-2">
            <CheckCircle className="w-4 h-4 text-bike shrink-0" /> Notificación enviada. Deberías recibirla ahora.
          </div>
        )}
      </div>
      ...
      <AnimatedButton 
        variant="secondary" 
        className="w-full text-xs font-black py-2.5 bg-surface-hover hover:bg-surface-card text-text-primary border border-border-default shadow-card cursor-pointer mt-4" 
        onClick={handleTest}
        disabled={loading}
      >
