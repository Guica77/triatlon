...
export function DeleteAccountCard({ scheduledFor }: { scheduledFor?: string | null }) {
  ...
  if (scheduledFor) {
    const date = new Intl.DateTimeFormat('es-ES', { day: 'numeric', month: 'long', year: 'numeric' }).format(new Date(scheduledFor));
    return (
      <div className="rounded-xl border border-warning/30 bg-bg-card p-5">
        <h3 className="text-sm font-bold text-text-primary">Eliminación programada</h3>
        <p className="mt-1 text-xs leading-relaxed text-text-muted">Tu cuenta y tus datos se eliminarán el {date}. Hasta entonces puedes cancelar la solicitud desde aquí.</p>
        <button type="button" disabled={pending} onClick={async () => { setPending(true); const result = await cancelAccountDeletion(); setPending(false); if (result.error) setError(result.error); else window.location.reload(); }} className="mt-4 min-h-11 rounded-lg border border-border-default px-4 py-2 text-sm font-bold text-accent">Cancelar eliminación</button>
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1 max-w-xl text-xs leading-relaxed text-text-muted">
            Programa la eliminación de tu perfil, chats, entrenamientos y datos asociados dentro de 30 días.
          </p>
        </div>
      ...
      {open && (
        <div className="fixed inset-0 z-[100] flex items-end justify-center bg-black/70 p-4 sm:items-center" role="presentation">
          <div role="alertdialog" aria-modal="true" aria-labelledby="delete-account-title" className="w-full max-w-md rounded-2xl border border-border-default bg-bg-card p-5 shadow-2xl">
            ...
            <h2 id="delete-account-title" className="mt-4 text-lg font-extrabold text-text-primary">¿Programar la eliminación de tu cuenta?</h2>
            <p className="mt-2 text-sm leading-relaxed text-text-muted">Cerraremos tu sesión y eliminaremos la cuenta dentro de 30 días. Puedes volver a iniciar sesión y cancelar la solicitud antes de esa fecha.</p>
            ...
            <div className="mt-5 flex gap-3">
              ...
              <button type="button" disabled={pending || confirmation !== 'ELIMINAR'} onClick={requestDeletion} className="flex flex-1 items-center justify-center gap-2 rounded-lg bg-danger px-4 py-3 text-sm font-bold text-white disabled:cursor-not-allowed disabled:opacity-40">
                {pending ? <Loader2 className="h-4 w-4 animate-spin" /> : <Trash2 className="h-4 w-4" />} Programar eliminación
              </button>
            </div>
          </div>
        </div>
