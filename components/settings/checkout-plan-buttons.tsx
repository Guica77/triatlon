    ...
    <div className="space-y-4">
      ...
      {showPaymentReview ? <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/40 p-0 backdrop-blur-sm sm:items-center sm:p-4" role="dialog" aria-modal="true" aria-labelledby="payment-review-title">
        <div className="w-full max-w-lg rounded-t-[28px] bg-surface-card p-5 shadow-2xl sm:rounded-[28px] sm:p-6">
          ...
          <button type="button" onClick={() => { setShowPaymentReview(false); void openEndpoint('/api/billing/checkout', selected, { plan: selected }); }} disabled={loading !== null} className="mt-5 flex min-h-12 w-full items-center justify-center gap-2 rounded-[14px] bg-accent px-5 text-[15px] font-semibold text-white disabled:opacity-60">{loading === selected ? <LoaderCircle className="h-4 w-4 animate-spin" /> : <LockKeyhole className="h-4 w-4" />}Continuar con Stripe</button>
          <button type="button" onClick={() => setShowPaymentReview(false)} className="mt-2 min-h-10 w-full rounded-xl text-sm font-semibold text-text-secondary hover:bg-surface-hover">Volver a elegir plan</button>
        </div>
      </div> : null}
