-- Apple product IDs are the authoritative source of coach capacity.
alter table public.admin_discount_campaigns
  add column if not exists product_identifier text;

update public.admin_discount_campaigns
set product_identifier = case
  when membership = 'athlete' then 'com.triwavex.athlete.monthly'
  when membership = 'coach' then 'com.triwavex.coach.monthly'
end
where product_identifier is null;

alter table public.admin_discount_campaigns alter column product_identifier set not null;

create unique index if not exists admin_discount_campaigns_product_discount_uidx
  on public.admin_discount_campaigns(product_identifier, discount_percent);

-- Keep all provider event ordering/idempotency checks from the hardened RPC,
-- while accepting only the explicitly configured Apple product IDs.
...
begin
  product_plan := case
    when p_provider_product_id = 'com.triwavex.athlete.monthly' then 'athlete'
    when p_provider_product_id = 'com.triwavex.coach.monthly'
      or p_provider_product_id ~ '^com\.triwavex\.coach\.monthly\.(15|20|25|30|35|40|45|50)$' then 'coach'
    else null end;
  if p_user_id is null or p_provider_reference is null
    or p_provider_transaction_reference is null or p_provider_signed_date is null
    or p_provider_event_type is null or p_provider_product_id is null
  ...
  if current_entitlement.user_id is not null then
    if current_entitlement.provider_signed_date > p_provider_signed_date then
      ...
      values('app_store',p_provider_transaction_reference,p_provider_reference,p_user_id,p_provider_signed_date,p_provider_event_type) on conflict(provider,transaction_reference) do nothing;
      return query select false,false,current_entitlement.status,p_user_id; return;
    end if;
    if current_entitlement.provider_signed_date = p_provider_signed_date then
      if current_entitlement.provider_transaction_reference = p_provider_transaction_reference and current_entitlement.provider_event_type = p_provider_event_type then return query select true,true,current_entitlement.status,p_user_id; end if;
      return query select false,false,current_entitlement.status,p_user_id; return;
    end if;
    terminal_event := current_entitlement.provider_event_type in ('REFUND','REVOKE','EXPIRED','GRACE_PERIOD_EXPIRED');
    if terminal_event and p_provider_event_type in ('SUBSCRIBED','DID_RENEW','OFFER_REDEEMED','RENEWAL_EXTENSION','RENEWAL_EXTENDED') then return query select false,false,current_entitlement.status,p_user_id; return; end if;
    if p_provider_event_type = 'REFUND_REVERSED' and (p_provider_expires_at is null or p_provider_expires_at <= now()) then return query select false,false,current_entitlement.status,p_user_id; return; end if;
  end if;
  insert into public.billing_provider_transactions(provider,transaction_reference,original_transaction_reference,user_id,provider_signed_date,event_type)
  values('app_store',p_provider_transaction_reference,p_provider_reference,p_user_id,p_provider_signed_date,p_provider_event_type) on conflict(provider,transaction_reference) do nothing;
  if current_entitlement.user_id is null then
    insert into public.billing_entitlements(user_id,plan,source,status,trial_ends_at,period_ends_at,provider_reference,provider_transaction_reference,provider_customer_reference,provider_signed_date,provider_environment,provider_product_id,provider_expires_at,provider_revocation_at,provider_event_type,provider_grace_period_ends_at,updated_at)
    values(p_user_id,p_plan,'app_store',p_status,null,p_period_ends_at,p_provider_reference,p_provider_transaction_reference,p_provider_customer_reference,p_provider_signed_date,p_provider_environment,p_provider_product_id,p_provider_expires_at,p_provider_revocation_at,p_provider_event_type,p_provider_grace_period_ends_at,now());
  else
    update public.billing_entitlements set plan=p_plan,source='app_store',status=p_status,trial_ends_at=null,period_ends_at=p_period_ends_at,provider_reference=p_provider_reference,provider_transaction_reference=p_provider_transaction_reference,provider_customer_reference=p_provider_customer_reference,provider_signed_date=p_provider_signed_date,provider_environment=p_provider_environment,provider_product_id=p_provider_product_id,provider_expires_at=p_provider_expires_at,provider_revocation_at=p_provider_revocation_at,provider_event_type=p_provider_event_type,provider_grace_period_ends_at=p_provider_grace_period_ends_at,updated_at=now()
  ...
  update public.profiles set subscription_status=p_status,
    active_plan_id=case when p_status='active' and p_plan='athlete' and p_active_plan_id is not null then p_active_plan_id when p_status<>'active' and p_plan='athlete' then null else active_plan_id end
  where id=p_user_id;
...
-- Accepting an invitation is serialized by coach and uses only the server-verified
-- Apple product ID to determine capacity; pending requests never use a seat.
